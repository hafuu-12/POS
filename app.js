...
let DB = {
  ...
  settings:{ shopName:'Ghousia Solar & Electric System', address:'', phone:'', gstRate:17, gstEnabled:false, logoData:'' },
...
function loadDB(){
  ...
  if(!DB.settings) DB.settings={shopName:'Ghousia Solar & Electric System',address:'',phone:'',gstRate:17,gstEnabled:false,logoData:''};
...
function buildReceipt(s){
  ...
  const logoHtml=st.logoData?`<img src="${st.logoData}" class="receipt-logo" alt="logo">`:'';
  ...
  tb.innerHTML=prods.map(p=>{
    ...
    return `<tr>
      ...
      <td><div style="display:flex;gap:4px">
        ${isAdmin?`<button class="btn btn-blue btn-sm" onclick="openEditProd(${p.id})">Edit</button>`:''}
        <button class="btn btn-success btn-sm" onclick="openRestock(${p.id})">+Stock</button>
        ${isAdmin?`<button class="btn btn-danger btn-sm" onclick="delProd(${p.id})">Del</button>`:''}
      </div></td>
    </tr>`;
  }).join('');
}
  ...
  reader.onload=function(e){
    ...
    for(let i=1;i<lines.length;i++){
      try{
        ...
        if(existing){Object.assign(existing,{name,sku,cat,buy,sell,stock,minStock,unit,icon,desc});updated++;}
        else{DB.products.push({id:DB.nextProdId++,name,sku,cat,buy,sell,stock,minStock,unit,icon,desc});added++;}
  ...
  g.innerHTML=custs.map(c=>{
    ...
    return `<div style="background:var(--card);border:2px solid var(--border);border-radius:13px;padding:15px;transition:.2s" onmouseover="this.style.borderColor='var(--g3)'" onmouseout="this.style.borderColor='var(--border)'">
      ...
      <div style="display:flex;gap:6px;margin-top:10px">
        ...
        <button class="btn btn-ghost btn-sm" onclick="openEditCust(${c.id})">Edit</button>
        <button class="btn btn-danger btn-sm" onclick="delCust(${c.id})">Del</button>
      </div>
    </div>`;
  }).join('');
}
...
function openEditCust(id){const c=DB.customers.find(x=>x.id===id);if(!c)return;editCust=c;document.getElementById('custModalTitle').textContent='Edit Customer';document.getElementById('cName').value=c.name;document.getElementById('cPhone').value=c.phone||'';document.getElementById('cCity').value=c.city||'';document.getElementById('cAddr').value=c.addr||'';document.getElementById('cNotes').value=c.notes||'';openModal('custModal');}
function saveCust(){
...
function renderSales(){
  ...
  tb.innerHTML=sales.map(s=>`<tr>
    ...
    <td><button class="btn btn-ghost btn-sm" onclick="viewSaleDet(${s.invoice})">View</button></td>
  </tr>`).join('');
}
...
function renderExp(){
  ...
  document.getElementById('expStats').innerHTML=`
    ...
    <div class="sc blue"><div class="si"></div><div class="sl">Top Category</div><div class="sv" style="font-size:16px">${topC?topC[0]:'—'}</div></div>
  `;
  ...
  tb.innerHTML=[...DB.expenses].reverse().map((e,ri)=>{const i=DB.expenses.length-1-ri;return `<tr>
    ...
    <td><div style="display:flex;gap:4px"><button class="btn btn-blue btn-sm" onclick="openEditExp(${i})">Edit</button><button class="btn btn-danger btn-sm" onclick="delExp(${i})">Del</button></div></td>
  </tr>`}).join('');
}
...
function openEditExp(i){const e=DB.expenses[i];editExp=i;document.getElementById('expModalTitle').textContent='Edit Expense';document.getElementById('expCat').value=e.cat;document.getElementById('expAmt').value=e.amount;document.getElementById('expDesc').value=e.desc;document.getElementById('expPaid').value=e.paidBy;document.getElementById('expDate').value=e.date;document.getElementById('expNote').value=e.note||'';openModal('expModal');}
function saveExp(){
...
function findReturnSale(){
  ...
  el.innerHTML=`<div style="background:var(--bg);border:1.5px solid var(--border);border-radius:10px;padding:14px">
    ...
    <button class="btn btn-danger" style="width:100%;margin-top:11px" onclick="openRetModal()">Process Return</button>
  </div>`;
}
function calcRetAmt(){
  if(!retSale)return;
...
function openRetModal(){
  ...
  document.getElementById('retItemList').innerHTML=sel.map(i=>`<div class="ret-item"><span>${i.icon||''} ${i.name} x${i.qty}</span><span style="font-family:JetBrains Mono,monospace;font-weight:700;color:var(--red)">Rs ${(i.price*i.qty).toLocaleString()}</span></div>`).join('');
...
function renderSettings(){
  document.getElementById('sName').value=DB.settings.shopName||'';
  document.getElementById('sAddr').value=DB.settings.address||'';
  document.getElementById('sPhone').value=DB.settings.phone||'';
  document.getElementById('gstRateInp').value=DB.settings.gstRate||17;
  updateGSTUI(); renderCatList(); renderLogoPreview();
}
...
function handleLogoUpload(event){
  ...
  reader.onload=function(e){
    ...
    saveDB(); renderLogoPreview(); applyLogoToSidebar();
...
function removeLogo(){
  ...
  DB.settings.logoData=''; saveDB(); renderLogoPreview(); applyLogoToSidebar();
  toast('Logo removed');
}
function renderLogoPreview(){
  ...
  if(DB.settings.logoData){
    preview.src=DB.settings.logoData; preview.style.display='block';
    placeholder.style.display='none'; removeBtn.style.display='inline-flex';
  } else {
    preview.style.display='none'; placeholder.style.display='flex'; removeBtn.style.display='none';
  }
}
function applyLogoToSidebar(){
  ...
  if(DB.settings.logoData){
    if(sideImg){sideImg.src=DB.settings.logoData;sideImg.style.display='block';}
    if(loginImg){loginImg.src=DB.settings.logoData;loginImg.style.display='block';}
  } else {
    if(sideImg){sideImg.src='g1.png';}
    if(loginImg){loginImg.src='g1.png';}
  }
}
